...
(function(){
  ...
  async function beginSignup(email,displayName){const bytes=crypto.getRandomValues(new Uint8Array(24)),temporaryPassword=`Haru!${Array.from(bytes,b=>b.toString(16).padStart(2,'0')).join('')}`;const {data,error}=await client.auth.signUp({email,password:temporaryPassword,options:{data:{display_name:displayName,signup_complete:false}}});fail(error,'인증 이메일 전송');currentUser=data.user||null;return data}
  async function completeSignup(password,displayName){const {data,error}=await client.auth.updateUser({password,data:{display_name:displayName,signup_complete:true,terms_accepted:'true',terms_version:'2026-09-09'}});fail(error,'회원가입 완료');currentUser=data.user||currentUser;return data}
  async function signInGuest(){const {data,error}=await client.auth.signInAnonymously({options:{data:{display_name:'게스트',test_guest:true}}});fail(error,'게스트 로그인');currentUser=data.user||null;return data}
  async function verifySignupOtp(email,token){const {data,error}=await client.auth.verifyOtp({email,token,type:'email'});fail(error,'이메일 인증');currentUser=data.user||null;return data}
  async function resendSignupOtp(email){const {data,error}=await client.auth.resend({type:'signup',email});fail(error,'인증번호 재전송');return data}
  ...
  async function getSource(type,id){if(!['record','diary','book'].includes(type))throw new Error('공유할 수 없는 항목입니다.');const {data,error}=await client.from('records').select('*,record_media(*)').eq('id',id).eq('user_id',uid()).single();fail(error,'공유 원본 불러오기');const expected=type==='diary'?'일상':type==='book'?'독서':null;if(expected&&data.category!==expected)throw new Error('공유 종류와 원본이 일치하지 않습니다.');if(type==='record'&&['일상','독서'].includes(data.category))throw new Error('이 항목은 해당 전용 화면에서 공유해 주세요.');return data}
  async function upsertPublication(payload){
    const {data,error}=await client.rpc('share_record_to_square',{p_source:payload.source_id,p_kind:payload.source_type,p_title:payload.title,p_body:payload.body||'',p_meta:payload.meta||{},p_cover_path:payload.cover_path||null});fail(error,'광장 공유');return data;
  }
